...
The project files are located in the `Garment-Store-Prototype/` directory.

- **Development**: `cd Garment-Store-Prototype && npm run dev` (runs on port 5000)
- **Production build**: `cd Garment-Store-Prototype && npm run build`
- **Production server**: `cd Garment-Store-Prototype && npm run start`
...
- **Environment Variables**:
  ...
  - `FRONTEND_URL`: (Optional) URL of the frontend if hosted separately.
  - `VITE_API_URL`: (Optional) Backend API URL for the React frontend.
  - `NODE_ENV`: Set to `production` for optimized builds and secure cookies.
